...
export function drawCaption(canvas: HTMLCanvasElement, caption: string) {
/*  
  const tmpCanvas = document.createElement('canvas');
  tmpCanvas.width = canvas.width;
  tmpCanvas.height = canvas.height;
  const tmpCtx = tmpCanvas.getContext('2d')!;
  tmpCtx.font = "bold 40px '源暎エムゴ'";
  ...
  tmpCtx.fillText(caption, canvas.width / 2, canvas.height * 3 / 4);

  tmpCtx.globalCompositeOperation = 'destination-over';
  tmpCtx.strokeStyle = "black";
  tmpCtx.lineWidth = 8;
  tmpCtx.lineJoin = 'round';
  tmpCtx.strokeText(caption, canvas.width / 2, canvas.height * 3 / 4);
